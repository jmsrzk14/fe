...
const DepartemenPage: React.FC = () => {
  ...
  const fetchData = async (pageNumber: number) => {
    ...
    try {
      ...
      let res = await fetch(
        `${API_URL}/admin/department?${params.toString()}`,
        {
          method: "GET",
          headers: {
  ...
  const handleEdit = (item: any) => {
      ...
      if (!resolvedId) {
        ...
        const match = data.find(
          (d) =>
            ...
            (item.short_name && d.shortname === item.short_name)
        );
    ...
    router.push(`/admin/department/edit?id=${resolvedId}`);
  };
  ...
  const handleDelete = async (item: Departemen) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/admin/department/${item.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
