...
const GaleryPage: React.FC = () => {
  ...
  const fields = [
    { key: "title", label: "Judul", type: "string" },
  ...
  const fetchData = async (pageNumber: number) => {
    ...
    try {
      ...
      let res = await fetch(
        `http://localhost:9090/api/admin/galery?${params.toString()}`,
        {
          method: "GET",
          headers: {
  ...
  // ✅ perbaikan di sini
  const handleAdd = () => {
    router.push("/admin/galery/create"); 
  };

  const handleEdit = (item: Galery) => {
    console.log("Edit item:", item);
  };

  const handleDelete = async (item: Galery) => {
    const token = sessionStorage.getItem("token");

    try {
      const res = await fetch(
        `http://localhost:9090/api/admin/galery/${item.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const LoadingState = () => (
    <div className="min-h-96 flex items-center justify-center bg-white rounded-xl shadow-sm border border-gray-100">
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-600 font-medium">Memuat data Galery...</p>
