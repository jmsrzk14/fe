...
const HimpunanPage: React.FC = () => {
  ...
  const searchParams = useSearchParams(); // <-- ditambahkan
  ...
  const fetchData = async (pageNumber: number) => {
    ...
    try {
      ...
      let res = await fetch(
        `${API_URL}/admin/association?${params.toString()}`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    ...
    const effectivePage = pageParam && !isNaN(pageParam) ? pageParam : 1;
    // sync state page supaya UI menampilkan benar
    if (effectivePage !== page) setPage(effectivePage);
    fetchData(effectivePage);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams?.get("page"), searchParams?.get("updated")]);
  ...
  const handleEdit = (item: any) => {
    ...
    if (typeof item === "number") {
      ...
      if (!resolvedId) {
        console.warn(
          "handleEdit: received index but failed to resolve id from data at index",
          item
        );
      }
      ...
      if (!resolvedId) {
        // try to find matching object in data by shallow match (name/short_name)
        const match = data.find(
          (d) =>
            (item.name && d.name === item.name) ||
            (item.short_name && d.short_name === item.short_name)
        );
    ...
    router.push(`/admin/himpunan/edit?id=${resolvedId}`);
  };
  ...
  const handleDelete = async (item: Himpunan) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/admin/associations/${item.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      await Swal.fire({
        icon: "success",
        title: "Berhasil!",
        text: `Data "${item.name}" telah dihapus.`,
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
            ...
            <nav className="flex items-center space-x-1">
              <button
                disabled={page === 1}
                onClick={() =>
                  router.push(`/admin/himpunan?page=${Math.max(1, page - 1)}`)
                }
              ...
              <div className="hidden sm:flex items-center space-x-1">
                {getPageNumbers().map((num, idx) =>
                  typeof num === "number" ? (
                    <button
                      key={idx}
                      onClick={() => router.push(`/admin/himpunan?page=${num}`)}
              ...
              <button
                ...
                onClick={() =>
                  router.push(`/admin/himpunan?page=${Math.min(totalPages, page + 1)}`)
                }
                className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 hover:shadow-sm"
              >
                Next
