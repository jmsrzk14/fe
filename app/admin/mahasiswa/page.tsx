...
interface Mahasiswa {
  ...
  period?: string;
  organization?: string; // Added to store organization name
}
...
const TableContainer: React.FC = () => {
  ...
  const [position, setPosition] = useState<string | null>(null); // To store organization name for selected student
  ...
  const baseRoleCategories: RoleCategory[] = [
    ...
    },
    {
      ...
      roles: [
        ...
        {
          value: "wakil_ketua_mpm",
          label: "Wakil Ketua MPM",
          icon: <Crown className="w-4 h-4" />,
          color: "from-yellow-400 to-yellow-500",
          description: "Membantu ketua dalam menjalankan tugas BEM"
        },
        {
          value: "sekretaris_mpm",
          label: "Sekretaris MPM",
          icon: <Shield className="w-4 h-4" />,
          color: "from-blue-500 to-blue-600",
          description: "Mengatur administrasi dan dokumentasi BEM"
        },
  ...
  const fetchOrganizations = async () => {
    ...
    try {
      // Fetch Himpunan
      const himpunanResponse = await fetch(`${API_URL}/association`, {
        method: "GET",
        headers: {
    ...
    // Fetch UKM
    try {
      const ukmResponse = await fetch(`${API_URL}/club`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    try {
      const departmentResponse = await fetch(`${API_URL}/department`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const fetchData = async (pageNumber: number) => {
    ...
    try {
      ...
      let res = await fetch(
        `${API_URL}/admin/students?${params.toString()}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  // Fetch student profile to get organization name
  const fetchStudentProfile = async (id: number) => {
    setLoadingOrganizations(true);

    try {
      const profilResponse = await fetch(`${API_URL}/admin/students/${id}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (!profilResponse.ok) throw new Error(`HTTP error! status: ${profilResponse.status}`);

      const profilResult: ProfileApiResponse = await profilResponse.json();
      const organizationName = profilResult?.data?.organization || "";
      setPosition(organizationName);
      return organizationName;
    } catch (err) {
      console.error("Gagal fetch student profile:", err);
      return "";
    } finally {
      setLoadingOrganizations(false);
    }
  ...
  const assignRole = async (studentId: number, role: string, category: string, period: string, user_id: number, organizationId?: number, organizationShortname?: string, positionTitle?: string) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/admin/students/${studentId}/assign`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (response.status === 200) {
        AlertToast.success("Mahasiswa berhasil di Assign.");
        setData(prevData =>
  ...
  const RoleAssignmentModal = ({
    ...
    const [organizationName, setOrganizationName] = useState<string>(""); // State for organization name

    // Fetch organization name when modal opens
    useEffect(() => {
      if (isOpen && student?.id) {
        fetchStudentProfile(student.id).then(name => setOrganizationName(name));
      }
    }, [isOpen, student?.id]);
                      ...
                      <div key={category.id} className="border border-gray-200 rounded-lg">
                        ...
                        {expandedCategory === category.id && (
                          <div className="border-t border-gray-200">
                            {(category.id === 'mpm' || category.id === 'bem') ? (
                              category.roles.map((role) => (
                                <button
                                  ...
                                  className={`w-full flex items-center justify-start p-3 text-left transition-all duration-200 hover:bg-gray-50 border-b border-gray-100 last:border-b-0 ${student.current_role === role.value && student.current_category === category.id && !student.current_organization_id
                                    ? 'bg-blue-50'
                                    : ''
                                      ...
                                      <div className="ml-12">
                                        {category.roles.map((role) => (
                                          <button
                                            ...
                                            disabled={assigningRole === student.id || !selectedPeriod}
                                            className={`w-full flex items-center justify-start p-3 text-left transition-all duration-200 hover:bg-gray-50 border-b border-gray-50 last:border-b-0 ${student.current_role === role.value &&
    ...
    <div className="space-y-6 p-6 bg-gradient-to-br from-gray-50 to-white min-h-screen">
      ...
      <RoleAssignmentModal
        ...
        onClose={() => {
          ...
          setPosition(null); // Reset organization name when modal closes
        }}
      />
