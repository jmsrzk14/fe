...
export default function UKMEditPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios
      .get(`${API_URL}/admin/clubs/${id}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      })
      .then((res) => {
        console.log("raw association response:", res.data);
        const payload = res.data.data || res.data;
        const { name, short_name, image } = payload;

        const previewUrl = image
          ? image.startsWith("http")
            ? image
            : `${IMAGE_URL}/clubs/${image}`
          : null;
  ...
  const handleBack = () => router.push("/admin/ukm");
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // kirim semua field, tidak peduli diubah atau tidak
      ...
      if (formData.image instanceof File) {
        ...
        // tetap kirim URL lama agar backend tahu tidak perlu hapus gambar
        body.append("imageUrl", formData.imageUrl);
      }

      const res = await axios.put(
        `${API_URL}/admin/clubs/${id}`,
        body,
        { headers }
      );

      if (res.status === 200) {
        AlertToast.success("Data ukm berhasil diperbarui");
        const ts = Date.now();
        router.push(`/admin/ukm?page=1&updated=${ts}`);
      }
    } catch (err) {
  ...
  const completenessPercent = Math.round(
    (((formData.name ? 1 : 0) +
      ...
      (formData.image || formData.imageUrl ? 1 : 0)) /
      3) *
    100
  );
          ...
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Perbarui data Unit Kegiatan Mahasiswa
              </p>
            </div>
            <button
              onClick={handleBack}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    disabled={submitting}
                  />
                  <div className="border-2 border-dashed border-blue-300 rounded-xl p-8 text-center gatos de puntajes
                    text-center hover:border-blue-500 hover:bg-blue-50 transition-all cursor-pointer bg-white">
                    {previewImage ? (
                      <div className="space-y-4">
                        <img
