   ...
   const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const info = JSON.stringify({
        ...
        os_version: `${os.name || "Unknown OS"} ${os.version || ""}`.trim(),
      });

      const campusPayload = {
        username: email,
        password,
        system_id: "f9c2f2b3-28d8-4b5a-a5b9-d2b5e8ef1234",
        platform: "Website BEM",
        info,
      };

      let data;
      let fromCampus = false;

      try {
        const campusRes = await fetch(`${API_URL}/auth/campus/login`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(campusPayload),
          credentials: "include",
        });

        if (campusRes.ok) {
          data = await campusRes.json();
          fromCampus = true;
        } else {
          throw new Error("Campus login gagal");
        }
      } catch (campusError) {
        const localRes = await fetch(`${API_URL}/auth/login`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ username: email, password }),
        });

        if (!localRes.ok) throw new Error("Login gagal ke sistem lokal");
      ...
      } else if (data.data?.role === "Mahasiswa" && !position) {
            ...
            <form className="space-y-4" onSubmit={handleLogin}>
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  />
                  <User
                    className="absolute right-5 top-3 h-4 w-4 text-black-400"
                    strokeWidth={3}
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2 mb-1">
                  ...
                  <Label htmlFor="password" className="text-[#1E40AF]">
                    Password
                  </Label>
                </div>
                <div className="relative">
                  ...
                  >
                    {showPassword ? (
                      <EyeOff className="h-4 text-black" strokeWidth={3} />
                    ) : (
                      <Eye className="h-4 text-black" strokeWidth={3} />
                    )}
                  </button>
                </div>
              </div>
              ...
              <Button className="w-full bg-[linear-gradient(to_right,#FFFFFF_0%,rgba(167,167,167,0.84)_30%,#3B82F9_70%,#60A5FA_100%)] hover:bg-[#2563EB]">
                ...
                <LogIn className="w-4 h-4 ml-2 text-white" strokeWidth={3} />
              </Button>
              ...
              <div className="flex items-center justify-center">
                  <div className="flex items-center space-x-2">
                    <Label
                      htmlFor="remember"
                      className="text-sm font-normal text-gray-400 mr-2"
                    >
                      Belum Memiliki Akun?
                    </Label>
                  </div>
