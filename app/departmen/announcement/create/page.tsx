...
export default function AnnouncementCreatePage() {
  ...
  useEffect(() => {
    ...
    const storedAuthorId = sessionStorage.getItem("id");
    const storedOrgId = sessionStorage.getItem("organization");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        router.push("/departmen/announcement");
      }
    } catch (err) {
  ...
  // Toolbar konfigurasi untuk ReactQuill
  const quillModules = {
    toolbar: [
      [{ header: [1, 2, 3, false] }],
  ...
  return (
    <div className="min-h-screen bg-white p-6 md:p-10">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <button
          onClick={() => router.push("/departmen/announcement")}
        ...
        >
          ...
          {/* React Quill untuk Konten */}
          <div>
            <label className="flex items-center gap-2 text-sm font-bold text-blue-900 mb-2">
              <FileText size={18} className="text-blue-600" /> Konten Pengumuman
            </label>
            <ReactQuill
              theme="snow"
