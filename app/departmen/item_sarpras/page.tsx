...
const BarangPage: React.FC = () => {
  ...
  const fields = [
    { key: "name", label: "Nama Barang", type: "string" },
    { key: "amount", label: "Kuantitas", type: "number" },
  ];
  ...
  const fetchData = async (pageNumber: number) => {
    ...
    try {
      ...
      let res = await fetch(
        `${API_URL}/student/item_sarpras?${params.toString()}`,
        {
          method: "GET",
          headers: {
  ...
  // ✅ perbaikan di sini
  const handleAdd = () => {
    router.push("/departmen/item_sarpras/create"); 
  };
  ...
  const handleDelete = async (item: Barang) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/student/item_sarpras/${item.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const LoadingState = () => (
    <div className="min-h-96 flex items-center justify-center bg-white rounded-xl shadow-sm border border-gray-100">
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-600 font-medium">Memuat data Barang...</p>
