...
const stripHtml = (html: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/admin/news`, formDataToSend, {
        headers: {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        const ts = Date.now();
        router.push(`/mpm/news?page=1&updated=${ts}`);
      }
    } catch (err) {
      const error = err as AxiosError<{ message?: string }>;
      console.error("POST /api/admin/news error:", error?.response ?? error);
                ...
                <div className="p-6 space-y-4">
                  ...
                  {formData.content && (
                    <div className="bg-blue-50 rounded-lg p-4">
                      <strong className="text-blue-700">Deskripsi:</strong>
                      <div
                        className="text-blue-700 text-sm leading-relaxed break-words line-clamp-3 mt-2 prose max-w-none"
                        // render HTML dari Quill — Quill sudah menghasilkan HTML, gunakan innerHTML
