...
export default function HimpunanDetail() {
  ...
  const [himpunanData, setHimpunanData] = useState<ApiHimpunan | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch himpunan basic data
        const himpunanResponse = await fetch(`${API_URL}/associations/${shortName}`);
        if (!himpunanResponse.ok) {
          throw new Error(`Failed to fetch himpunan data: ${himpunanResponse.status}`);
        }
        const himpunanResult: HimpunanApiResponse = await himpunanResponse.json();
        if (himpunanResult.status === 'success' && himpunanResult.data) {
          setHimpunanData(himpunanResult.data);
        }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-red-50">
        <div className="text-center bg-white p-8 rounded-2xl shadow-md max-w-md">
          ...
          <h1 className="text-xl font-bold text-red-600 mb-2">Himpunan Tidak Ditemukan</h1>
          <p className="text-gray-700 mb-4">
            Data himpunan <span className="font-semibold">{shortName}</span> tidak ditemukan dalam sistem.
          </p>
          <button
            onClick={() => window.location.href = '/user/organization'}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
          >
            Kembali ke Daftar Himpunan
          </button>
        </div>
      </div>
    );
  ...
  if (adminData) {
    if (adminData.leader) members.push({ member: adminData.leader, position: 'ketua_himpunan' });
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <div className="flex flex-col items-center mt-8 space-y-6">
            <div className="w-32 h-32 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center overflow-hidden shadow-2xl hover:scale-110 transition-transform duration-300">
              <img
                src={`${IMAGE_URL}/associations/${himpunanData.image}`}
                alt={himpunanData.name}
                className="w-24 h-24 object-cover rounded-full"
                onError={(e) => {
                  const target = e.target as HTMLImageElement;
                  target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(himpunanData.short_name)}&background=ffffff&color=3b82f6&size=200`;
                }}
              />
            </div>
            <div className="text-center space-y-2">
              <h1 className="text-5xl font-extrabold tracking-tight drop-shadow-lg">{himpunanData.short_name}</h1>
              <p className="text-xl opacity-90 max-w-2xl">{himpunanData.name}</p>
            </div>
      ...
      <div className="container mx-auto px-6 py-16 space-y-16">
        ...
        <section>
          ...
          >
            ...
            <h2 className="text-4xl font-bold text-gray-800 mb-3">Pengurus Himpunan</h2>
