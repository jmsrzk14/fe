...
interface DepartemenApiResponse {
  data: ApiDepartemen;
  message: string;
  status: string;
}
...
const positionLabels: Record<string, string> = {
  'ketua_department': 'Ketua Departemen',
...
export default function DepartemenDetail() {
  ...
  const [departemenData, setDepartemenData] = useState<ApiDepartemen | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch departemen basic data
        const departemenResponse = await fetch(`${API_URL}/associations/${shortName}`);
        if (!departemenResponse.ok) {
          throw new Error(`Failed to fetch departemen data: ${departemenResponse.status}`);
        }
        const departemenResult: DepartemenApiResponse = await departemenResponse.json();
        if (departemenResult.status === 'success' && departemenResult.data) {
          setDepartemenData(departemenResult.data);
        }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-red-50">
        <div className="text-center bg-white p-8 rounded-2xl shadow-md max-w-md">
          ...
          <h1 className="text-xl font-bold text-red-600 mb-2">Departemen Tidak Ditemukan</h1>
          <p className="text-gray-700 mb-4">
            Data departemen <span className="font-semibold">{shortName}</span> tidak ditemukan dalam sistem.
          </p>
          <button
  ...
  if (adminData) {
    if (adminData.leader) members.push({ member: adminData.leader, position: 'ketua_department' });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-slate-900 via-gray-800 to-zinc-900 text-white py-16 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <div className="flex flex-col items-center mt-8 space-y-6">
            <div className="w-32 h-32 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center overflow-hidden shadow-2xl hover:scale-110 transition-transform duration-300">
              <img
                src={`${IMAGE_URL}/departments/${departemenData.image}`}
                alt={departemenData.name}
                className="w-24 h-24 object-cover rounded-full"
                onError={(e) => {
                  const target = e.target as HTMLImageElement;
                  target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(departemenData.short_name)}&background=ffffff&color=3b82f6&size=200`;
                }}
              />
            </div>
            <div className="text-center space-y-2">
              <h1 className="text-5xl font-extrabold tracking-tight drop-shadow-lg">{departemenData.short_name}</h1>
              <p className="text-xl opacity-90 max-w-2xl">{departemenData.name}</p>
            </div>
      ...
      <div className="container mx-auto px-6 py-16 space-y-16">
        ...
        <section>
          ...
          >
            ...
            <h2 className="text-4xl font-bold text-gray-800 mb-3">Pengurus Departemen</h2>
