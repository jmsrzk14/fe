...
export default function SarprasForm() {
  ...
  useEffect(() => {
    const fetchInventaris = async () => {
      try {
        const res = await fetch(`${API_URL}/item_sarpras`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedItems.length === 0) {
      alert("Pilih setidaknya satu barang inventaris!");
      return;
    }
    ...
    // 🔧 Gunakan FormData agar bisa kirim file + data teks
    ...
    // 🔁 Barang bisa dikirim sebagai JSON array
    form.append("items", JSON.stringify(selectedItems));

    // 🖼️ File upload (jika ada)
    ...
    try {
      const res = await fetch(`${API_URL}/student/requests_sarpras`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
        body: form, // FormData, bukan JSON.stringify
      });

      const responseText = await res.text();
      console.log("📨 Respon dari server:", responseText);

      if (res.ok) {
        alert("🎉 Peminjaman berhasil diajukan!");
        router.push("/user/service");
      } else {
        console.error("❌ Gagal:", responseText);
        alert(`Gagal mengajukan: ${responseText}`);
      }
    } catch (error) {
